...
function CollectionCard({ col }: { col: Collection }) {
    ...
    return (
        ...
        >
            ...
            <div>
                <p className="text-sm font-medium text-gray-900 dark:text-white mb-1">{col.name}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed line-clamp-2">{col.description}</p>
            </div>
            <p className="text-[11px] text-gray-400 dark:text-gray-500">Updated {col.updatedAt}</p>
        </div>
    );
}
    ...
    return (
        <div className="flex flex-col h-full bg-gray-50 dark:bg-neutral-900 min-w-0">
            ...
            <header className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-neutral-800 bg-gray-50/90 dark:bg-neutral-900/90 backdrop-blur-md flex-shrink-0">
                <div>
                    <h1 className="text-sm font-semibold text-gray-900 dark:text-white tracking-tight">Collections</h1>
                    <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-px">{COLLECTIONS.length} collections</p>
                </div>
