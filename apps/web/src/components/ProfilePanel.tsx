...
export default function ProfilePanel() {
  ...
  const inputCls =
    'w-full bg-white dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-400 transition-all';
  const labelCls =
            ...
            {tab === 'profile' && (
              <>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/50">
                        ● Active
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex gap-2.5">
                  <button className="h-8 px-4 rounded-lg text-xs font-semibold text-white bg-indigo-500 hover:bg-indigo-400 transition-colors">
                    Save changes
                  </button>
                  <button className="h-8 px-4 rounded-lg text-xs font-medium text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-neutral-700 hover:bg-gray-100 dark:hover:bg-neutral-800 transition-colors">
                    Discard
                  </button>
                </div>
            ...
            {tab === 'preferences' && (
              <>
                {[
                  ...
                  {
                    label: 'Editor font',
                    options: ['JetBrains Mono', 'Fira Code', 'Cascadia Code'],
                  },
                ].map(({ label, options }) => (
                  <div key={label}>
                ...
                <button className="h-8 px-4 w-fit rounded-lg text-xs font-semibold text-white bg-indigo-500 hover:bg-indigo-400 transition-colors">
                  Save preferences
                </button>
              </>
            )}
          ...
          <div className="flex-1 flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              {STAT_CARDS.map(({ value, label }) => (
                <div
                  key={label}
                  className="bg-white dark:bg-neutral-800/50 border border-gray-200 dark:border-neutral-700 rounded-xl p-5"
                >
